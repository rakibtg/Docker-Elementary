...
module.exports = {
  fetchContainers: (status) => new Promise( async (resolve, reject) => {
  ...
  fetchContainerByID: containerID => new Promise(async (resolve, reject) => {
    const containerInspect = await cmdLikeAPro('docker container inspect ' + containerID)
    const result = lightContainerDetail(containerID, JSON.parse(containerInspect)[0])
    resolve(result)
  })
}